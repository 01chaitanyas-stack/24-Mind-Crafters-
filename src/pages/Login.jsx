...
const STYLE_ID = '__login-keyframes';
...
input[type="email"] { -webkit-appearance:none; appearance:none; background-image:none !important; background-clip:unset !important; }
input[type="email"]::-webkit-contacts-auto-fill-button,
input[type="email"]::-webkit-credentials-auto-fill-button { display:none !important; visibility:hidden !important; pointer-events:none !important; width:0 !important; height:0 !important; }
input[type=email]::-webkit-inner-spin-button,
input[type=email]::-webkit-cancel-button,
input[type=email]::-webkit-search-decoration,
input[type=email]::-webkit-textfield-decoration-container { display:none!important; visibility:hidden!important; pointer-events:none!important; }
input:-webkit-autofill,
input:-webkit-autofill:hover,
input:-webkit-autofill:focus { -webkit-box-shadow:0 0 0px 1000px rgba(255,255,255,0.04) inset !important; -webkit-text-fill-color:#f1f1f1 !important; border-color:rgba(255,255,255,0.1) !important; }
  `;
...
export default function Login() {
  ...
  const onBlurReset = e => {
    e.target.style.borderColor = isLight ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.1)';
  ...
  return (
    <div style={{ minHeight: '100vh', background: isLight ? '#f5f0e8' : '#0c0c14', position: 'relative', overflow: 'hidden', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 20px 80px', transition: 'background 0.4s ease, color 0.3s ease' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', marginBottom: 36, opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(28px)', transition: 'opacity 0.7s, transform 0.7s' }}>
        ...
        <h1 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 42, letterSpacing: '-1px', color: isLight ? '#1a1a2e' : '#fff', margin: '0 0 12px', lineHeight: 1.1, transition: 'color 0.3s ease' }}>
          Welcome Back
        </h1>
        <p style={{ color: isLight ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.35)', fontSize: 15, margin: 0, lineHeight: 1.6, maxWidth: 340, marginLeft: 'auto', marginRight: 'auto', transition: 'color 0.3s ease' }}>
          Sign in to access your personalized Nashik journey
        </p>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: 460, background: isLight ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.03)', border: isLight ? '1px solid rgba(0,0,0,0.08)' : '1px solid rgba(255,255,255,0.08)', borderRadius: 28, padding: 36, backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', boxShadow: isLight ? '0 20px 60px rgba(0,0,0,0.12)' : '0 40px 80px rgba(0,0,0,0.5)', opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(28px)', transition: 'opacity 0.7s 0.2s, transform 0.7s 0.2s, background 0.4s ease, border-color 0.4s ease' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, color: isLight ? '#1a1a2e' : '#fff', fontSize: 16, transition: 'color 0.3s ease' }}>Secure Login</div>
            <div style={{ color: isLight ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.3)', fontSize: 12, marginTop: 2, transition: 'color 0.3s ease' }}>Your session is encrypted</div>
          </div>
        </div>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ animation: 'slideIn 0.5s 0.4s both' }}>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                onFocus={e => { e.target.style.borderColor = 'rgba(245,158,11,0.5)'; e.target.style.boxShadow = '0 0 0 3px rgba(245,158,11,0.12)'; }}
                onBlur={onBlurReset}
              />
          ...
          {error && (
            <div style={{ background: 'rgba(239,68,68,0.10)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 999, padding: '10px 18px', color: '#f87171', fontSize: 13, fontFamily: "'DM Sans', sans-serif", textAlign: 'center' }}>
              {error}
            </div>
          )}
        ...
        <p style={{ marginTop: 24, textAlign: 'center', fontSize: 13, color: isLight ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.3)', fontFamily: "'DM Sans', sans-serif", transition: 'color 0.3s ease' }}>
          ...
          <Link to="/signup" style={{ color: isLight ? '#c17f24' : '#f59e0b', fontWeight: 700, textDecoration: 'none' }}>
            Create one
          </Link>
        </p>
      </div>
